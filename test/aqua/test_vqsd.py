...
from qiskit.aqua.algorithms import VQSD


@ddt
class TestVQSD(QiskitAquaTestCase):
    """ Test VQSD """
    def setUp(self):
        super().setUp()
        self.seed = 50
    ...
    @unpack
    def test_vqsd_optimizers(self, optimizer_cls, places, max_evals_grouped):
        """ VQSD Optimizers test """
        result = VQSD(self.qubit_op,
                     RYRZ(self.qubit_op.num_qubits),
                     optimizer_cls(),
    ...
    def test_vqe_qasm(self):
        ...
        algo = VQSD(self.qubit_op, var_form, optimizer, max_evals_grouped=1)
    ...
    def test_vqsd_statevector_snapshot_mode(self):
        """ VQSD Aer statevector_simulator snapshot mode test """
        try:
    ...
    def test_vqe_qasm_snapshot_mode(self):
        """ VQSD Aer qasm_simulator snapshot mode test """
        try:
            # pylint: disable=import-outside-toplevel
            from qiskit import Aer
        except Exception as ex:  # pylint: disable=broad-except
    ...
    def test_vqsd_callback(self):
        ...
        def store_intermediate_result(eval_count, parameters, global_obj,
                                      local_obj, weighted_obj):
            with open(self.get_resource_path(tmp_filename), 'a') as file:
                content = "{},{},{:.5f},{:.5f},{:.5f}".format(eval_count, parameters,
        ...
        algo = VQSD(self.qubit_op, var_form, optimizer,
    ...
    def test_vqsd_reuse(self):
        ...
        num_qubits = self.qubit_op.num_qubits
        var_form = RY(num_qubits, 3)
        vqsd.var_form = var_form
        with self.assertRaises(AquaError):
            _ = vqsd.run()

        vqsd.operator = self.qubit_op
        with self.assertRaises(AquaError):
            _ = vqsd.run()

        qinst = QuantumInstance(BasicAer.get_backend('statevector_simulator'))
        vqsd.quantum_instance = qinst
        result = vqsd.run()
        ...
        operator = MatrixOperator(np.array([[1, 0, 0, 0],
                                            ...
                                            [0, 0, 2, 0],
                                            [0, 0, 0, 3]]))
        vqsd.operator = operator
        result = vqsd.run()
        self.assertAlmostEqual(result.eigenvalue.real, -1.0, places=5)

    def test_vqsd_mes(self):
        """ Test VQSD minimum eigen solver interface """
        vqsd = VQSD(var_form=RY(self.qubit_op.num_qubits, 3), optimizer=COBYLA())
        vqsd.set_backend(BasicAer.get_backend('statevector_simulator'))
        result = vqsd.compute_minimum_eigenvalue(self.qubit_op)
        self.assertAlmostEqual(result.eigenvalue.real, -1.85727503, places=5)
